		...
		for (int i = 0; i < edgeMatrix.size(); i++) {
			if (i != matrixId) {
				if (edgeMatrix[i][matrixId] != NULL) {
					if (edgeMatrix[i][matrixId]->dst->vertexId != vertexId) {
						if (max < edgeMatrix[i][matrixId]->dst->vertexId)
							max = edgeMatrix[i][matrixId]->dst->vertexId;
					}
					else {
						if(max < edgeMatrix[i][matrixId]->src->vertexId)
						max = edgeMatrix[i][matrixId]->src->vertexId;
					}
				}
			}
		}
...
int main() {
	...
	for (int i = 0; i < t; i++) {
		...
		if (s == "MaxAdjacentNode") {
			cin >> sr;
			g.MaxAdjacentNode(sr);
		}
	}
}